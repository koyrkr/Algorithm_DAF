...
//construct dag data structure
    int* visited = (int *)malloc(numQueryNode * sizeof(int));
    memset(visited, 0, sizeof(int) * numQueryNode);
    ...
    //print result of aDFS
    for (int i = dagEnd - 1; i > -1; i--) {
      printf("%d ", dagQuery[i]);
    }
...
void aDFS(int v, int *visited) {
    int* children = (int *) malloc((adjIndexQuery[v + 1] - adjIndexQuery[v] + 1) * sizeof(int));
    int childrenEnd = 0;

    visited[v] = 1;

    // make array for children
    for (int i = adjIndexQuery[v]; i < adjIndexQuery[v + 1]; ++i) {
        children[childrenEnd++] = adjListQuery[i];
    }

    // sort children
    merge_sort(children, 0, childrenEnd - 1, sortByDegreeQuery);
    merge_sort(children, 0, childrenEnd - 1, sortByLabelFreqQuery);

    // loop through children
    for (int i = 0; i < childrenEnd; i++) {
        int child = children[i];

        if(visited[child] == 0) {
            aDFS(child, visited);
            dagChildQuery[v][ dagChildQuerySize[v] ] = child;
            dagParentQuery[child][ dagParentQuerySize[child] ] = v;

            ++dagChildQuerySize[v];
            ++dagParentQuerySize[v];
        }
    }
...
void clearMemory(){
      ...
      if(dagQuery != NULL) free(dagQuery);
    	if(dagChildQuery != NULL) {
        	for(int i = 0; i < numQueryNode; ++i) {
            		if(dagChildQuery[i] != NULL)
                		//delete[] dagChildQuery[i];
